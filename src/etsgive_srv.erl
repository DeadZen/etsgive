...
handle_cast(count, State) ->
        ...
        Result = ets:update_counter(TableId, count, 1),
        io:format("Counter: ~p~n", [Result]),
        {noreply, State};
handle_cast(check, State) ->
        TableId = State#state.table_id,
        io:format("TableId: ~p Data: ~p~n", [TableId, ets:tab2list(TableId)]),
        {noreply, State};
...
handle_info({'ETS-TRANSFER', TableId, Pid, _Data}, State) ->
    io:format("MGR(~p) -> SRV(~p) getting TableId: ~p~n", [Pid, self(), TableId]),
